...
for i=60:10:180
    ...
    if k==1%Insufficient data
        ...
        Di(Di<=0)=[];
        [phat,pci]=gamfit(Di);
        a=phat(1)./5;%phat(1)./5 based on characteristics based on gamma distribution
    end
    PM(k,1)=a;
    PM(k,2)=phat(2);
    ...
    if k==1
        w=[0.5 0 0.5];%Insufficient data to calculate consistency and weights
    else
...
xlswrite('Credibility and membership of Gamma.xlsx',[CE CRE']);
xlswrite('Calculation time of Gamma.xlsx',ET);
...
p=[];
for t=1:1:300
    l=(L-xi)/b:0.1:(L-xi)/b+100;
    y=(log(l)-psi(a.*t)).*(l.^(a.*t-1)).*exp(-l);
    Y=trapz(l,y);
    f=(a* Y)/gamma(a*t);
    p=[p f];
end
...
function A=KSE(Data,a,b)
di=sort(Data);
step=0.0001;
totalNum=length(di);
y1=cdf('Gamma',0:step:1,a,b);
y0=[];
for j=0:step:1
